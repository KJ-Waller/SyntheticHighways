    ...
    def parse_trajectories(self, xml_fname):
        ...
        for vehicle in vehicles:
            ...
            for i, pos in enumerate(vehicle):
                ...
                trajectory[i] = lat, lon, speed, x, y, z, heading, timestamp, gt_segment, path_id, pathpos_idx, vehicle_type
                t_len += 1
            ...
            idxs_vtype = np.argwhere(veh_types != 'Car')
            if idxs_vtype.size > 0:
                splits_vtype = np.split(trajectory, idxs_vtype, axis=0)


                idxs = np.sort(np.concatenate([idxs_speed, idxs_vtype]))

                splits = np.split(trajectory, idxs_vtype, axis=0)
